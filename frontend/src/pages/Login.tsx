...
export default function Login({ onLogin }: { onLogin: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        // Spring Boot returns { message: "..." } on error
        throw new Error(data.message ?? 'Invalid credentials');
      }

      // Store token and user info for all future API calls
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPass(!showPass)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-steel/60 hover:text-steel">
            ...
            {error && (
              <p className="text-red-400 text-xs font-body bg-red-400/10 border border-red-400/20 rounded-lg px-3 py-2">
                {error}
              </p>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-amber text-navy font-body font-semibold py-2.5 rounded-lg text-sm hover:bg-amber/90 transition-colors disabled:opacity-60 mt-2"
            >
              {loading ? 'Signing in...' : 'Sign In'}
            </button>
          </form>

          <p className="text-center text-xs text-steel/50 mt-6 font-body">admin@nitc.ac.in / admin123</p>
        </div>
      </div>
    </div>
  );
